...
def import_all_models() -> None:
    """Recursively import all models modules within the package."""
    backend_path = Path(__file__).resolve().parent.parent
    for path in backend_path.iterdir():
...
def include_object(object, name, type_, reflected, compare_to):  # type: ignore[no-untyped-def]
    """Prevent Alembic from touching tables that are not in SQLAlchemy"""
    if type_ == "table" and name.startswith("auth_"):
        return False
    if type_ == "table" and name.startswith("django_"):
        return False
    return True
